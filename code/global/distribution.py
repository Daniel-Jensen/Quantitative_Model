...
def forward_iterate(D, a_pol, a_grid, Pi):
    ...
    # Single np.bincount over flattened (a, e) indices (~4x faster than per-e add.at).
    n_a, n_e = D.shape
...
def forward_paths(D0, a_pol_path, c_path, a_grid, Pi, use_fast=True):
    ...
    # Timing: C_t on the START-of-period dist, A_t on the END-of-period one;
    # D_start[t] is the dist entering period t (D_start[0]=D0, D_start[T]=terminal).
    import fast_kernels
