...
SEM=[prctile(Y,5),prctile(Y,95)];%SEM_calc(Y); %Supplied external function
SD=[prctile(Y,5),prctile(Y,95)];%nanstd(Y);  %Requires the stats toolbox 
mu=median(Y); %Requires the stats toolbox 

%The plot colors to use for multiple sets of points on the same x
%location
cols=hsv(length(X)+1)*0.5;
cols(1,:)=0;
jitScale=jitter*1; %To scale the patch by the width of the jitter
...
for k=1:length(X)
    ...
    if strcmp(style,'patch') 
      h(k).sdPtch=patchMaker(SD(k,:),[0.8,0.8,0.8]);
    end
    
    
    %===================================================
    % THIS CONTROLS THE WITH OF THE LINE FOR THE MEDIAN
    %===================================================
    if strcmp(style,'patch') || strcmp(style,'sdline')
      % CONTROLS THE COLOR OF THE BAR FOR THE BANDS
     % h(k).semPtch=patchMaker(SEM(k),[1,0.6,0.6]);
      % CONTROLS THE COLOR OF THE MEDIAN LINE
      h(k).mu=plot([X(k)-jitScale,X(k)+jitScale],[mu(k),mu(k)],'color',[0,0.0500,0.7000],...
           'linewidth',4.5);
    end
