function [policies,expectations,residuals,prem] = model_policies(param,gamma,bounds,EXP,TT,coll_points,ss,s,V,eps)

% This function generates the model policy functions 
%
...
% expectations (matrix collecting objects for LTROs experiment),
% residuals (vector collecting the residuals at solution)
%
...
gam_1(:,j)   = ((exp_11(j,:)/TT)*(TT_exp.*EXP))';
gam_2(:,j)   = ((exp_22(j,:)/TT)*(TT_exp.*EXP))';
gam_3(:,j)   = ((exp_33(j,:)/TT)*(TT_exp.*EXP))';
gam_4(:,j)   = ((exp_44(j,:)/TT)*(TT_exp.*EXP))';
