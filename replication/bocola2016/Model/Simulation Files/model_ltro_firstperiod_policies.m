function [residuals,expectations,PREMIUM,RISK_comp,MULT_comp,reject,policies] = model_ltro_firstperiod_policies(param,gamma,bounds,coll_points,TT,s,ss,EXP,V,expectations,prem,eps,m)
...
qq(:,j)      = q';
RR(:,j)      = R';
mu(:,j)      = ((N_tom+m)./(lambda*(Q.*K_tom+q.*B_tom-m)));
mu1(:,j)     = 1./(lambda*(Q.*K_tom+q.*B_tom-m));
mu2(:,j)     = ((Q.*K_tom+q.*B_tom)-m)./(Q.*K_tom+q.*B_tom-m);
...
mu               = max(mu2-(((1-psi)+psi*R_impl.*(exp_2./cc.^(-1))).*mu) + psi*(exp_5./cc.^(-1)).*mu1,0);
...
reject(aa(:,1)*m<=psi*(exp_5(:,1)./cc(:,1).^(-1))) = 1;
