function [residuals] = residual_model_open(param,gamma,bounds,EXP,TT,coll_points,ss,s)
...
N_tom   =  Theta*(k_ret.*exp(ss(6)+x(1,:)) + b_ret.*exp(ss(end-1)+x(5,:)) -exp(ss(8)+x(3,:))) + ...
            omega*(Q.*exp(ss(6)+x(1,:))+q.*exp(ss(end-1)+x(5,:)).*(1-def(j)*rec)+psi.*(1-alpha).*gdp);
...
exp_1        = log(beta*(cons-((chi*lab.^(1+(1/nu)))/(1+(1/nu)))).^(-sigma));

exp_2        = log(exp(exp_1).*alp);

exp_3        = log((exp(exp_1).*((1-Theta)+Theta*alp).*k_ret));

exp_4        = log((exp(exp_1).*((1-Theta)+Theta*alp).*b_ret));

prem(j,:)    = (k_ret);

gam_1(:,j)   = ((exp_1/TT)*(TT1.*EXP))';

gam_2(:,j)   = ((exp_2/TT)*(TT1.*EXP))';

gam_3(:,j)   = ((exp_3/TT)*(TT1.*EXP))';

gam_4(:,j)   = ((exp_4/TT)*(TT1.*EXP))';
...
mu               = min(max(1-(((1-Theta)+Theta*RR.*(exp_2./(cc-((chi*ll.^(1+(1/nu)))/(1+(1/nu)))).^(-sigma))).*mu),0),0.99);
