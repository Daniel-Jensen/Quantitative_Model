...
| Parameter | Status |
|---|---|
| `Delta_bD_D`, `Delta_bF_F` (own) | **CORRECTED 2026-08-17 — live value is 0.20, not 0.80.** The `Delta_own > ~0.73 (D) / ~0.71 (F)` bound below was derived at the **CT1 scope**, where `phi_own = 2.390`. Under the live `BANK_SCOPE="broad"` the concentration ratio is 0.456, the feasibility condition `f*theta > (1-Delta_own)*phi_own + (1-Delta_cross)*phi_cross` slackens, and `Delta_own = 0.20` passes `assert_gk_well_posed` on every solved SS. So the bound does **not** bind at the live scope and `Delta_own` is currently a **free inherited parameter with no live lower bound and no moment**. Verify against `get_calibration()`, not against this table's history. |
| `Delta_bF_D`, `Delta_bD_F` (cross) | **CORRECTED 2026-08-17 — live value is 0.40, not 0.90.** Still unidentified. `Delta_own < Delta_cross` is preserved (0.20 < 0.40): each bank finds its **own** sovereign the better collateral, a home-bias convention (home repo access, home-jurisdiction eligibility) that should be stated in the paper rather than assumed. |
