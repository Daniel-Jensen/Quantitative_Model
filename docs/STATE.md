...
> **This repository carries TWO solvers of the same model and this file logs
> both.** Part I is the sequence-space (SSJ) pipeline in `code/*.py` — the
> linearised sticky-price / nominal-deposit model that `experiments/` (E1–E4),
> `diagnostics/regimes/` and the paper's current figures run on. Part II is the
> global-projection pipeline in `code/global/` — the nonlinear
> occasionally-binding solution. They share no code, no calibration and no
> interpreter. **Numbers are not comparable across the two parts**; always say
> which pipeline a figure came from. See CLAUDE.md §Environment.
>
> The two logs were merged when `bocola-rewrite` was reconciled with `main`
> (2026-09-14). Neither part was edited in that merge; each is the branch's own
> record, kept verbatim.

---

# Part I — Sequence-space pipeline (`code/*.py`)
...
# Part II — Global-projection pipeline (`code/global/`)
