...
| **GK-2** | **OPEN — dynamic instability at measured concentration.** With the SS correct, `b_gov_D[499] ~ 1e2–1e3` (target ~1e-5). Amplification `theta*phi_own = 13.17` vs `4.0*0.25 = 1.0` for the placeholder. **Not** fiscal (flat in `phi_lamb` up to 25) and **not** the collateral friction (present at `psi_lambda_B=0`); `mv_rule=1` doesn't fix it. `chi1` 0→0.5 cuts peak spread 1.1e7bp→6.0bp but no value in [0.2,5.0] removes the root. Three routes in `docs/eba_calibration.md` "Dynamic instability". |
