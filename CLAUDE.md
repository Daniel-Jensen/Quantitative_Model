...
| **GK-2** | **PARTLY FIXED (2026-07-31).** `omega_K` as a *fixed share* made the passive fund mirror bank deleveraging, so `dK/dN = theta/omega_K` (47.1 at measured net worth). New `fund_rule_D/F=1`: the fund holds a constant `K_fund`, bank is marginal holder, `dK/dN = theta` (5.5). **Steady state identical** (`lambda_gk_D=+0.9271`, `K_D=10.800` under both rules); at `omega_K=1` the rules coincide so pre-EBA is untouched. `b_gov[499]` improves 17x but is still explosive. Two *measured* amplifiers remain: thin net worth (`n_inter` 3.0->0.41 takes `b_gov[499]` 1e-8 -> 1.85) and concentration (`phi_own` 0.25->2.39 takes it 1.85 -> 532). Ruled out: `phi_lamb`, `psi_lambda_B`, `mv_rule`, `Delta`, and the sovereign-risk schedule (`def_scale`->0 is *worse*, so a bounded `tanh` cannot help). See `docs/eba_calibration.md`. |
